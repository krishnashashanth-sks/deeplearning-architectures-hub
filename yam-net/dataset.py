import pandas as pd
from utils import download_audioset_clip,waveform_to_log_mel_spectrogram,SAMPLE_RATE
import tensorflow as tf
import pandas as pd
import io
import librosa

# The label data provided by the user
label_data_string = """
0,/m/09x0r,Speech
...
labels_df = pd.read_csv(io.StringIO(label_data_string), header=None, names=['index', 'mid', 'display_name'])

# Save the DataFrame to a CSV file
output_csv_path = 'yamnet_class_labels.csv'
labels_df.to_csv(output_csv_path, index=False)

# Create a dummy AudioSet-like CSV content for demonstration
dummy_csv_content = """
YT_ID,start_seconds,end_seconds,positive_labels
dQw4w9WgXcQ,20.000,20.960,"/m/0463cq4"
"""

# Save the dummy content to a temporary CSV file
dummy_csv_path = 'dummy_audioset_segments.csv'
with open(dummy_csv_path, 'w') as f:
    f.write(dummy_csv_content)

# Read the dummy CSV file into a pandas DataFrame
audioset_df = pd.read_csv(dummy_csv_path, skipinitialspace=True)

# Iterate through the DataFrame and download each clip
# For real AudioSet, you might want to process in batches or parallelize
print("Starting dummy audio downloads...")
for index, row in audioset_df.iterrows():
    youtube_id = row['YT_ID']
    start_time = row['start_seconds']
    end_time = row['end_seconds']

    # YAMNet typically processes 0.96s clips, so we ensure the duration is correct
    # If the original segment is longer, we can just use the start_time and AUDIO_DURATION
    # For this example, we'll use the provided end_time
    download_audioset_clip(youtube_id, start_time, end_time, output_dir='downloaded_audio_clips')

print("Dummy audio download process finished.")
print(f"Check the 'downloaded_audio_clips' directory for downloaded files.")

# Path to the downloaded dummy audio file
audio_file_path = 'downloaded_audio_clips/dQw4w9WgXcQ_20.0-20.96.wav'

# Load the audio file using librosa
# Make sure to resample to the SAMPLE_RATE expected by the preprocessing function
try:
    audio_waveform, sr = librosa.load(audio_file_path, sr=SAMPLE_RATE, mono=True)
    print(f"Loaded audio: {audio_file_path} with sample rate {sr} Hz and duration {len(audio_waveform)/sr:.2f} seconds.")

    # Preprocess the waveform into a log Mel-spectrogram
    # The waveform_to_log_mel_spectrogram function expects a tf.Tensor
    log_melspec_processed = waveform_to_log_mel_spectrogram(tf.constant(audio_waveform, dtype=tf.float32), SAMPLE_RATE)

    # YAMNet model expects an input shape of (batch, time_steps, features, channels)
    # We need to add a batch dimension and a channel dimension
    X_train_single_example = tf.expand_dims(tf.expand_dims(log_melspec_processed, axis=0), axis=-1)

    print(f"Preprocessed Log Mel Spectrogram shape for training: {X_train_single_example.shape}")

    # You can now use X_train_single_example as an input to your model
    # For a real dataset, you would collect many such examples into a batch

except FileNotFoundError:
    print(f"Error: Audio file not found at {audio_file_path}. Please ensure the download completed successfully.")
except Exception as e:
    print(f"An error occurred during audio processing: {e}")

# Get the input shape from the preprocessed example
input_spectrogram_shape = X_train_single_example.shape[1:]  # (94, 64, 1)
num_classes = 521 # As defined in build_yamnet_model

# Create dummy data for training, validation, and testing
# For demonstration, we'll use small batch sizes
BATCH_SIZE = 16

# Dummy X_train, X_val, X_test (Mel-spectrograms)
X_train = tf.random.uniform(shape=[BATCH_SIZE, *input_spectrogram_shape], minval=log_melspec_processed.numpy().min(), maxval=log_melspec_processed.numpy().max(), dtype=tf.float32)
X_val = tf.random.uniform(shape=[BATCH_SIZE // 2, *input_spectrogram_shape], minval=log_melspec_processed.numpy().min(), maxval=log_melspec_processed.numpy().max(), dtype=tf.float32)
X_test = tf.random.uniform(shape=[BATCH_SIZE // 2, *input_spectrogram_shape], minval=log_melspec_processed.numpy().min(), maxval=log_melspec_processed.numpy().max(), dtype=tf.float32)

# Dummy y_train, y_val, y_test (one-hot encoded labels)
# Since YAMNet is multi-label, use random binary values (0 or 1)
y_train = tf.cast(tf.random.uniform(shape=[BATCH_SIZE, num_classes], minval=0, maxval=2, dtype=tf.int32), dtype=tf.float32)
y_val = tf.cast(tf.random.uniform(shape=[BATCH_SIZE // 2, num_classes], minval=0, maxval=2, dtype=tf.int32), dtype=tf.float32)
y_test = tf.cast(tf.random.uniform(shape=[BATCH_SIZE // 2, num_classes], minval=0, maxval=2, dtype=tf.int32), dtype=tf.float32)
